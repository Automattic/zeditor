...
var normalizer = require('../editor-normalizer');
...
/**
 * Creates and returns a new `Editor` instance with the given `wrapper` DOM
 * element. 
 * *
...
function Editor(wrapper, options) {
  if (!(this instanceof Editor)) return new Editor(wrapper, options);
  ...
  this.normalizer = normalizer(this);
  ...
  // initialize default Editor plugins
...
Editor.prototype.placeholder = function(placeholder) {
...
Editor.prototype.getParam = deprecate(function (k){
...
Editor.prototype.setParam = deprecate(function(k, v){
...
Editor.prototype.block = function(el) {
...
Editor.prototype.normalizeSelection = function() {
...
/**
 * React to the Editor instance's "selectionchange" event.
 *
 * @private
 */
...
Editor.prototype.oncontentchange = function() {
  this.checkEmpty();
  this.normalizeSelection();
...
Editor.prototype.execute = function(name, val) {
...
Editor.prototype.showDebugger = function(e) {
  e.preventDefault();
  this.htmldebugger.toggle();
};

/**
 * returns `true` if there's no "content" inside the Editor, `false` otherwise
 *
 * @return {Boolean}
 * @public
 */
...
Editor.prototype.isEmpty = function() {
  // fast check: if there's < 2 childNodes in Editor, then it's potentially empty
...
Editor.prototype.checkEmpty = function() {
  debug('checkEmpty()');
...
Editor.prototype.ondragstart = function(el) {
...
Editor.prototype.ondragcommit = function(el, split) {
  if (is.element(el) && classes(el).has('overlay-reference')) {
    var overlay = this.overlay.for(el);
...
Editor.prototype.ondragcancel = function(el) {
  if (is.element(el) && classes(el).has('overlay-reference')) {
    var overlay = this.overlay.for(el);
    classes(overlay).remove('dragging');
  }
...
Editor.prototype.mousetrapStopCallback = function (e, element) {
  while (element) {
    if (element == this.el) {
      return false;
    }
...
Editor.prototype.incrementTasks = function(n){
  n = n || 1;
  this.tasks += n;
...
Editor.prototype.decrementTasks = function(n){
  n = n || 1;
  this.tasks -= n;
...
/**
 * Adds the given CSS string to the `<head>` of the Editor's document node.
 *
 * The CSS is may be scoped to the "Editor" instance by using the `::editor`
...
Editor.prototype.addStyles = function (styles) {
...
/**
 * Creates and returns a new "Editor Tip" instance with the given HTML string.
 *
 * Useful for editor plugins.
 *
...
Editor.prototype.tip = function (html) {
